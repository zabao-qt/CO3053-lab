...
uint32_t timer4_counter = 0;
uint32_t timer4_mul = 0;
...
void timer4_set_ms(int ms) {
    ...
    uint64_t mul = ((uint64_t)ms * 1000ULL) / tick_us;
    if (mul == 0) mul = 1; // minimum 1 tick
    if (mul > 0xFFFFFFFFU) mul = 0xFFFFFFFFU;
    timer4_mul = (uint32_t)mul;
    timer4_counter = timer4_mul;
    timer4_flag = 0;
}

void timer4_set_us(int us) {
    uint32_t tick_us = TIMER_CYCLE_4_US;
    if (tick_us == 0) tick_us = 1000;
    uint64_t mul = (uint64_t)us / tick_us;
    if (mul == 0) mul = 1;
    if (mul > 0xFFFFFFFFU) mul = 0xFFFFFFFFU;
    timer4_mul = (uint32_t)mul;
    timer4_counter = timer4_mul;
